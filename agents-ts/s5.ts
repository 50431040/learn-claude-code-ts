...
const temp = matter(
  fs.readFileSync(
    path.join(__dirname, "../skills/mcp-builder/SKILL.md"),
    "utf-8",
  ),
);
...
class SkillLoader {
  ...
  getDescriptions() {
    return Object.entries(this.skills)
      .map(([name, skill]) => `- ${name}: ${skill.meta}`)
      .join("\n");
  }
